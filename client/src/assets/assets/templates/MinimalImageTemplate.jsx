import { Mail, Phone, MapPin, HandshakeIcon, Globe, BriefcaseBusiness } from "lucide-react";
          ...
          <section className="mb-8">
            ...
            <div className="space-y-2 text-sm">
              ...
              {data.personal_info?.profession && (
                <div className="flex items-center gap-2">
                  <BriefcaseBusiness size={14} style={{ color: accentColor }} />
                  <span className="font-medium">{data.personal_info.profession}</span>
                </div>
              )}
              {data.personal_info?.linkedin && (
                <div className="flex items-center gap-2">
                  <HandshakeIcon size={14} style={{ color: accentColor }} />
            ...
            <section className="mb-8">
              ...
              <div className="space-y-4 text-sm">
                {data.education.map((edu, index) => (
                  <div key={index}>
                    <p className="font-semibold uppercase">{safeString(edu.degree)}</p>
                    <p className="text-zinc-600">{safeString(edu.institution)}</p>
            ...
            <section>
              ...
              <ul className="space-y-1 text-sm">
                {data.skills.map((skill, index) => (
                  <li key={skill?._id || index}>{typeof skill === "string" ? skill : safeString(skill?.name)}</li>
                ))}
              </ul>
            </section>
          )}
                ...
                {data.experience.map((exp, index) => (
                  <div key={index}>
                    <div className="flex justify-between items-center">
                      <h3 className="font-semibold text-zinc-900">{safeString(exp.position)}</h3>
            ...
            <section>
              ...
              <div className="space-y-4">
                {data.projects.map((project, index) => (
                  <div key={index}>
                    <h3 className="text-md font-medium text-zinc-800 mt-3">{safeString(project.name)}</h3>
                    <p className="text-sm mb-1" style={{ color: accentColor }}>
                      {safeString(project.type)}
                    </p>
